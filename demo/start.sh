...
go build -o ../build/demo_motif ../cmd/motif

rm -f ./transactions.rlp
for ((i=0;i<$N;i+=1))
do
    DATADIR="${PWD}/motif$i.datadir"
    rm -fr ${DATADIR}
    mkdir -p ${DATADIR}
	...
	--nousb --verbosity=3 --tracing &>> motif$i.log)&
...
attach_and_exec() {
    ...
    do
        ...
        res=$(../build/demo_motif --exec "${CMD}" attach http://127.0.0.1:${RPCP} 2> /dev/null)
        if [ $? -eq 0 ]
        then
            #echo "success" >&2
            echo $res
