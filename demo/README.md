...
Contains the scripts to do motif benchmarking (only for fakenet).
...
```sh
go run ../cmd/motif attach http://localhost:4000
```
...
* Get node1 address:
```sh
go run ../cmd/motif attach --exec "ftm.accounts[0]" http://localhost:4001
```
...
```sh
go run ../cmd/motif attach --exec "ftm.getBalance(ftm.accounts[0])" http://localhost:4000
go run ../cmd/motif attach --exec "ftm.getBalance(ftm.accounts[0])" http://localhost:4001
```
 outputs:
```js
